...
files_name_list_to_read = {
                            "source":"./global_nat_source.match_zone", 
...
if src_max != dst_max:
  print "source max : %s, destination max : %s are not matched" % (str(src_max), str(dst_max))
  sys.exit(0)
...
if len(sorted_source) != len(sorted_destination):
  print "sourted source : %s, sourted destination : %s are not matched" % (str(len(sorted_source)), str(len(sorted_destination)))
  sys.exit(0)
...
if len(sorted_by_zone_source) != len(sorted_by_zone_destination):
  print "sourted source : %s, sourted destination : %s are not matched" % (str(len(sorted_by_zone_source)), str(len(sorted_by_zone_destination)))
  sys.exit(0)
...
for i_ in range(policy_rule_number):
   ...
   for _source_value_ in searching_object_matched_ruld_number(policy_index, sorted_by_zone_source):
      for _destination_value_ in searching_object_matched_ruld_number(policy_index, sorted_by_zone_destination):
         ...
         f.write("IP address in the Group : \n")
         ...
         f.write("\n")
         f.write("Global Outbound NAT IP Address : \n")
         ...
         # service combination
         all_member_string = "\t".join([str(policy_index), rule_status, _source_value_[0], source_members_string, _destination_value_[0], desti_members_string])
